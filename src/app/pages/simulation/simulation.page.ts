...
export class SimulationPage{
  ...
  slideOptsOne = {
    ...
    speed: 1000,
    onInit: (slides: any) =>
      this.slideOptsOne = slides
   };
   
 
  constructor(private http: HttpClient, private authService: AuthService, private toastCtrl: ToastController,
    private db: AngularFireDatabase, private modalCtrl: ModalController,) {
      this.getStarted();
    }

    getData() {
      this.http.get(`https://financialmodelingprep.com/api/v3/historical-price-full/${this.stock}?from=${this.start}&to=${this.end}&apikey=b14b67761bf2ef55e3ec5bcce3343f37`).subscribe(res => {
  ...
  async getStarted() {
    ...
    await this.getTSLAStocks().then(value => {
      TSLA = value as TSLA[];
    });
    ...
    await this.getMSFTStocks().then(value => {
      MSFT = value as MSFT[];
    });
  ...
  getTSLAStocks(){
    return new Promise((resolve, reject)=>{
      this.db.list('TSLA').valueChanges().subscribe(value => {   //CHANGE Stock/AAPL -> Stock/AnyCompanyTicker to get that company or just Stocks to get all companies
        resolve(value);
      });
    });
  }
  ...
  get canEnter(){
    return this.stock && this.start && this.end;
  }
